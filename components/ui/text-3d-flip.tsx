...
const splitIntoCharacters = (text: string, locale = "ar"): string[] => {
  if (HAS_SEGMENTER) {
...
const Text3DFlip = ({
  ...
  // Use appropriate locale for segmenter
  const segmenterLocale = useMemo(() => (isArabic ? "ar" : "en"), [isArabic])

  const characters = useMemo(() => {
    // Split text into words (handle both LTR and RTL)
    const words = text.split(/\s+/)
    return words.map((word, i) => ({
      characters: splitIntoCharacters(word, segmenterLocale),
      needsSpace: i !== words.length - 1,
    }))
  }, [text, segmenterLocale])
  ...
  return (
    <ElementTag
      className={cn(
        ...
        isArabic && "flex-row-reverse justify-end", // RTL support for flex container
        className
      )}
...
// Adjusted transforms for consistent behavior in both LTR and RTL
